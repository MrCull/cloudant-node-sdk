...
printf ">>>>> Publishing typedoc for release build: repo=%s branch=%s build_num=%s job_name=%s" ${GIT_REPO} ${BRANCH_NAME} ${BUILD_NUMBER} ${JOB_NAME}
printf ">>>>> Cloning repository's gh-pages branch into directory 'gh-pages'"
git clone --branch=gh-pages https://${GH_CREDS}@github.com/IBM/cloudant-node-sdk.git gh-pages
